...
export function KpiCard({ icon: Icon, label, value, trend, trendLabel, accent = "navy" }) {
  ...
  return (
    <div className="relative bg-white rounded-xl border border-border shadow-card p-4 lg:p-5 flex flex-col gap-3 overflow-hidden animate-rise hover-lift group">
      <span
        className={cn(
          "absolute -top-8 -right-8 h-24 w-24 rounded-full opacity-10 group-hover:opacity-20 transition-opacity duration-300",
          glow[accent]
        )}
      />
      <div className="flex items-center justify-between relative z-10">
        <div className={cn("h-10 w-10 rounded-[10px] flex items-center justify-center text-white shadow-sm", accents[accent])}>
          <Icon size={18} />
        </div>
        {trend !== undefined && (
          <span
